...
const ProjectImage = ({ flip, image, url }: CProjectImageProps) => {
  return (
    <motion.a href={url} target="_blank" whileHover={{ y: 10 }} className={`relative ${flip && "order-last"} lg:w-3/4 xl:w-1/2 cursor-pointer`}>
      <motion.div className={`h-full w-full absolute top-4 ${flip ? "left-4" : "right-4"} border-4  border-red-400  rounded`}></motion.div>

      <Image src={image} layout="fill" objectFit="cover" objectPosition="center" alt="headhsot" className=" relative rounded" />
